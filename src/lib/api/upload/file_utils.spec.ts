  ...
  return {
    readFile: jest.fn().mockImplementation((path, cb) => {
      if (path === './../logo_png.png') {
        cb(null, Buffer.from('teststring'));
      } else if (path === './../logo_png.1.png') {
        cb({ 'errno': -2, 'code': 'ENOENT', 'syscall': 'open', 'path': 'testDirectory/logo_png.1.png' }, null);
      }
    }),
...
describe('api:upload:file_utils', () => {
  ...
  describe('getFile', () => {
    ...
    it('should properly reject when unable to read a file', (done) => {
      ...
      expect.assertions(1);
      const expected = { 'errno': -2, 'code': 'ENOENT', 'syscall': 'open', 'path': `testDirectory/logo_png.1.png` };
    ...
    it('should set application/octet-stream if mimetype not found', (done) => {
      const buffer = Buffer.from('teststring', 'base64');
      expect.assertions(1);
      const expected = {
        buffer,
        'name': undefined,
